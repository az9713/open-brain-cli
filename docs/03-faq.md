...
### "Do I need to deploy the Edge Function?"

Only if you use GUI clients (Claude Desktop, ChatGPT, Cursor). If you only use terminal-based AI tools (Claude Code, Codex, Gemini CLI), the `ob` CLI handles everything — no Edge Function, no MCP server. Both paths use the same `thoughts` table and are fully interoperable, so you can add MCP later if needed.
...
*This FAQ is based on real community questions from the Substack chat and YouTube comments. If you run into something not covered here, [open an issue](https://github.com/az9713/open-brain-cli/issues) or post in the [Substack community](https://natesnewsletter.substack.com/) so everyone benefits from the answer.*
